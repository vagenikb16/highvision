...
const projects = [
  { id: "b_5x8prfdt4", clip: "https://www.youtube.com/watch?v=wtstKVV4Ha8" },
  { id: "asqi2HzDk_Q", clip: "https://www.youtube.com/watch?v=C481KpGKMC0" },
  { id: "F3pgQt7CZVs", clip: "https://www.youtube.com/watch?v=uchIN_cMCqo" },
  { id: "o-s2v8sR5P8", clip: "https://www.youtube.com/watch?v=HO44Ksuryzw" },
  { id: "u2G64HVROAM", clip: "https://www.youtube.com/watch?v=Gpb-4BVsj7o" },
  { id: "xkGj8iGfqOc", clip: "https://www.youtube.com/watch?v=XTmZaSztLIo" },
  { id: "FjNzYuOXebI", clip: "https://www.youtube.com/watch?v=4BrZqfGMUso" },
  { id: "a-okfop9C9Q", clip: "https://www.youtube.com/watch?v=72-pLfXakZs" },
  { id: "REStcz-DIdA", clip: "https://www.youtube.com/watch?v=fHrKK8GivDI" },
  { id: "irpuNkbKLH8", clip: "https://www.youtube.com/watch?v=e_U-wSo1o9s" },
  { id: "0CXmjsKWOQQ", clip: "https://www.youtube.com/watch?v=Qlw_LdFCdIU" },
  { id: "imfZcE_-17E", clip: "https://www.youtube.com/watch?v=zzw98xiFUTY" },
  { id: "s4pNsni5OOE", clip: "https://www.youtube.com/watch?v=Z0iZTX7mVL4" },
  { id: "Yv9FkAoULGc", clip: "https://www.youtube.com/watch?v=4iBG161G3cM" },
  { id: "dKi9_hLiMKY", clip: "https://www.youtube.com/watch?v=hH2XVkkmvmI" },
  { id: "d8M7tfxO6l0", clip: "https://www.youtube.com/watch?v=f2tBjKQdzFk" },
];
...
function generateBackstageGrid() {
  ...
  if (window.innerWidth <= 768) {
    let out = '';
    out += `<div class="backstage-video-full bv-tap" data-video-id="${vIds[0]}"><img class="bv-thumb" src="https://i.ytimg.com/vi/${vIds[0]}/hqdefault.jpg" loading="lazy" alt="backstage video 1"><div class="bv-play"></div></div>`;
    for (let i = 1; i <= 6; i++) out += `<div class="backstage-item small-p"><img src="${import.meta.env.BASE_URL}foto/${i}.jpg" loading="lazy"></div>`;
    out += `<div class="backstage-video-full bv-tap" data-video-id="${vIds[1]}"><img class="bv-thumb" src="https://i.ytimg.com/vi/${vIds[1]}/hqdefault.jpg" loading="lazy" alt="backstage video 2"><div class="bv-play"></div></div>`;
    for (let i = 7; i <= 12; i++) out += `<div class="backstage-item small-p"><img src="${import.meta.env.BASE_URL}foto/${i}.jpg" loading="lazy"></div>`;
    return out;
  }
  ...
  return layout.map(type => {
    if (type === 'v' && vIdx < 2) {
      const id = vIds[vIdx++];
      return `<div class="backstage-item big-v"><iframe src="https://www.youtube.com/embed/${id}?autoplay=1&mute=1&loop=1&playlist=${id}&controls=0&modestbranding=1&rel=0" frameborder="0" loading="lazy" allow="autoplay; encrypted-media" allowfullscreen title="backstage-desktop-video"></iframe></div>`;
    } else if (pIdx <= 16) {
      return `<div class="backstage-item small-p"><img src="${import.meta.env.BASE_URL}foto/${pIdx++}.jpg" loading="lazy"></div>`;
    }
    return '';
  }).join('');
}
...
const i18n = {
  en: {
    ...
    interlude_tag:"BEYOND THE FRAME", interlude_headline:"WHERE VISION<br><em>TAKES SHAPE</em>",
    interlude_body:"A perfect shot is the result of a second of chaos, the silence between takes, and the tension on set. We invite you behind the scenes: to where the concept becomes reality. This is not just a gallery, this is a story of how our art is created."
  },
  ua: {
    ...
    interlude_tag:"ЗА МЕЖЕЮ КАДРУ", interlude_headline:"ТАМ, ДЕ БАЧЕННЯ<br><em>НАБУВАЄ ФОРМИ</em>",
    interlude_body:"Ідеальний кадр — це результат секундного хаосу, тиші між дублями та напруги на майданчику. Запрошуємо вас за лаштунки: туди, де концепт стає реальністю. Це не просто галерея, це історія про те, як створюється наше мистецтво."
  }
};
...
function render() {
  ...
  const cards = projects.map((p,i) => {
    const tag = p.clip ? 'a' : 'div';
    const attrs = p.clip ? `href="${p.clip}" target="_blank" rel="noopener noreferrer"` : '';
    return `<${tag} class="item ${i<4?'wide':''}" ${attrs}>
      <div class="v-frame">
        <iframe src="${yt(p.id)}" loading="lazy" title="video-${i}"></iframe>
        <img class="v-thumb" src="https://i.ytimg.com/vi/${p.id}/hqdefault.jpg" loading="lazy" alt="video preview">
  ...
  document.querySelector('#app').innerHTML = `
    ...
    <section class="hero">
      ...
      <div id="hero-logo"><span class="hero-line1">HIGH</span><span class="hero-line2">VISION</span></div>
    ...
    <footer id="contact">
      <div class="footer-main">
        ...
        <div id="form-container" class="concept-form">
          <div class="form-wrapper"><iframe data-src="${googleFormUrl}" width="100%" height="1000" frameborder="0">Loading...</iframe></div>
        </div>
      </div>
...
function attachEvents() {
  document.querySelectorAll('.lang-btn').forEach(b => { b.onclick = () => { currentLang = b.dataset.lang; document.documentElement.lang = currentLang === 'en' ? 'en' : 'uk'; render(); }; });
  document.querySelectorAll('.bv-tap').forEach(el => {
    el.onclick = () => {
      const vid = el.dataset.videoId;
      el.innerHTML = `<iframe src="https://www.youtube.com/embed/${vid}?autoplay=1&mute=0&controls=1&modestbranding=1&rel=0&playsinline=1" frameborder="0" allow="autoplay; encrypted-media" allowfullscreen title="backstage-video"></iframe>`;
      el.classList.remove('bv-tap');
      el.onclick = null;
    };
  });
  ...
  if (ctaTrigger) ctaTrigger.onclick = () => {
    ...
    const frame = form.querySelector('iframe[data-src]');
...
function init3D() {
  ...
  window.onmousemove = (e) => {
    ...
    requestAnimationFrame(() => {
      const x = (lastX - window.innerWidth/2)/40;
      const y = (lastY - window.innerHeight/2)/40;
